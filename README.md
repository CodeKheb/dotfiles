...
│                           # treesitter, lspconfig, barbar, nvim-tree,
│                           # colors, luasnip, livepreview, autopairs…
└── after/snippets/         # Custom snippets: html.lua, java.lua
```

### Features
- **Native LSP** via `nvim-lspconfig` + **mason.nvim** for server installs
- **LSP servers enabled:** `clangd`, `rust-analyzer`, `gopls`, `ts_ls`, `pyright`, `bashls`, `lua_ls`, `templ`, `tailwindcss`, `html`, `cssls`, `kotlin_language_server`, `yaml-language-server`, `jdtls`
...
- **Telescope** finder, **Harpoon** quick navigation, **undotree**, **fugitive** (Git), **barbar** buffer tabs, **nvim-tree** file explorer
...
| Terminal | `<leader>t` | Open vertical terminal |
...
| Tools | `<leader>e` | Toggle nvim-tree |
...
| `$mod+Delete` | Power off |
| `$mod+Shift+Delete` | Reboot |
...
- `ctrl+t` → tmux overlay, `ctrl+enter` → nvim overlay (current dir)
...
| barbar.nvim, gitsigns.nvim | Buffer tabs with git status |
